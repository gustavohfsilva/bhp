...
if ($_POST["c_valor"]) {        
        ...
        $sql = "select * from cadastro_pessoa where cd_id ='{1}' AND cd_tipo='Fidelidade' ";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_array($result);
        extract($row);
        
        echo "$id<br>$valor <br> $fpg_id<br> <br>$cd_nome";
        die;
        }
         if($cd_limite > $valor) {
            echo "<script type='text/javascript'>
            ...
            $titulo = "Compra";
            $tipo = $cod3;
            $msg = "$cd_nome possui o limite fidelide de: R$ $cd_limite e está tentando compra$valor <p><b>Compra não Autorizada<b/></p>";
        } 
